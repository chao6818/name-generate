...
function getReferenceItems() {
  ...
  const addReference = (char, source) => {
    const known =
      ...
      [...BABY_CHARS.male, ...BABY_CHARS.female].find((item) => item.c === char);
    refs.push({
      char,
      source,
...
function getBirthLabel() {
  return `${state.birthYear}年${state.birthMonth}月 · ${getZodiac(state.birthYear)}年`;
}
...
function getCandidatePool() {
  let pool = state.gender === "neutral" ? [...BABY_CHARS.male, ...BABY_CHARS.female] : BABY_CHARS[state.gender];
...
function pickGivenChars(rng, usedChars = new Set()) {
  ...
  const length = state.length === "short" ? 1 : state.length === "long" ? 2 : rng() > 0.35 ? 2 : 1;
  const chosen = [];
  let attempts = 0;
...
function renderReferenceStrip() {
  ...
  strip.hidden = false;
  strip.innerHTML = `<span class="ref-pill"><strong>${escapeHtml(getBirthLabel())}</strong></span>` + parts
    .map((part) => `<span class="ref-pill"><strong>${escapeHtml(part.source)}名字中的「${escapeHtml(part.text)}」</strong></span>`)
    .join("");
}
...
function bindEvents() {
  ...
  document.getElementById("birthYear").addEventListener("change", (event) => {
    state.birthYear = Number(event.target.value);
    generateNames();
  });

  document.getElementById("birthMonth").addEventListener("change", (event) => {
    state.birthMonth = Number(event.target.value);
    generateNames();
  });
